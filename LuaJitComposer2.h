...
namespace Lua
{
    // JitComposer generates LuaJIT bytecode from a non-Lua frontend.
    //
    // LuaJIT's JIT compiler (lj_record.c) was designed for bytecode produced by
    // the standard Lua parser, which allocates registers strictly sequentially
    // (LIFO stack discipline). The following rules ensure that externally
    // generated bytecode stays compatible with the JIT recorder:
    //
    // 1. SEQUENTIAL ALLOCATION: Always allocate the next slot at
    //    highestUsedSlot + 1. Do NOT reuse gaps (holes) left by freed slots.
    //    Reason: the JIT tracks slots via J->maxslot and stale TRef markers
    //    in freed-but-not-cleared slots can cause type mismatches and
    //    frame chain corruption in rec_check_slots().
    //
    // 2. CALL FRAME LAYOUT: For CALL instructions, allocate a contiguous block
    //    of getCallFrameSize() + numArgs slots starting above all live values.
    //    Layout: [base]=func, [base+cfs..base+cfs+n-1]=args, where
    //    cfs = getCallFrameSize() (1 for non-FR2, 2 for FR2/GC64 on x64).
    //    The extra FR2 slot between func and first arg is the frame link slot
    //    managed internally by the VM -- do not write to it.
    //
    // 3. FRAMESIZE: The framesize reported in the prototype header must be >=
    //    the highest slot ever allocated + 1. The VM, GC, and JIT all use
    //    framesize to bound stack scanning and to place continuation frames
    //    for metamethods. An incorrect framesize can cause silent corruption.
    //    Use closeFunction(pool.d_frameSize) to pass the tracked high-water mark.
    //
    // 4. JMP OPERAND A: The A operand of JMP encodes a slot hint that the JIT
    //    uses to shrink J->maxslot at branch points (fewer slots in snapshots).
    //    Pass the current high-water mark (highestUsedSlot + 1) rather than
    //    the function-wide framesize for better JIT snapshot efficiency.
    //
    // 5. AFTER CALL RETURNS: Return values are placed at the call base slot.
    //    All slots above the return values (the former argument slots) become
    //    dead. MOV the result to a dedicated slot and release the call frame
    //    immediately to keep the live range tight.
    //
    ...
    public:
        ...
        static int currentHighWater( const SlotPool& pool ); // highestUsedSlot + 1, for JMP operand A
