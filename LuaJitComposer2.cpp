...
bool JitComposer::KSET(SlotNr dst, const QVariant& v, quint32 line )
{
    ...
    else if( JitValue::isPrimitive( v ) )
        return addAd(JitBytecode::OP_KPRI, dst, JitValue::toPrimitive(v), line );
...
{
    if( JitValue::isString(rhs) )
        return addAd(JitBytecode::OP_USETS, toUv, getConstSlot(rhs), line );
    else if( JitValue::isNumber(rhs) )
        return addAd(JitBytecode::OP_USETN, toUv, getConstSlot(rhs), line );
    else if( JitValue::isPrimitive(rhs) )
        return addAd(JitBytecode::OP_USETP, toUv, JitValue::toPrimitive(rhs), line );
    Q_ASSERT( false );
    return false;
}
...
bool JitComposer::ISEQ(SlotNr lhs, const QVariant& rhs, quint32 line)
{
    ...
    else if( JitValue::isString(rhs) )
        return addAd(JitBytecode::OP_ISEQS, lhs, getConstSlot(rhs), line );
    else if( JitValue::isPrimitive(rhs) )
        return addAd(JitBytecode::OP_ISEQP, lhs, JitValue::toPrimitive(rhs), line );
    Q_ASSERT( false );
    return false;
}
...
bool JitComposer::ISNE(SlotNr lhs, const QVariant& rhs, quint32 line)
{
    if( JitValue::isNumber(rhs) )
        ...
        return addAd(JitBytecode::OP_ISNEP, lhs, JitValue::toPrimitive(rhs), line );
    Q_ASSERT( false );
    return false;
}
